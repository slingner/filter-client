...
export default class BeanPage extends React.Component {
  ...
  fetchBeanReviews = () => {
    FilterApiService.getBeanReviews(this.props.id).then((reviews) => {
  ...
  deleteBean = (id) => {
    const newBeans = this.props.userBeans.filter((beans) => beans.id !== id);
    this.setState({
      beans: newBeans,
    });
  ...
  // fetch delete call to delete specific bean card based on bean id
  deleteReview = (id) => {
    const newReviews = this.context.reviews.filter(
